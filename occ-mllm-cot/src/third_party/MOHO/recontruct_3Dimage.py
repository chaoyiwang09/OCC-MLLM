...
from models.obman_dataset_ddp_dino import MultiObjectObmanDataset, MultiObjectObmanDataset_Single, get_iter_sampler
...
def knn_loss(rays_o, normal, k_normal):
    ...
    kp = torch.gather(xyz.unsqueeze(1).expand(-1, xyz.size(1), -1, -1), 2,
...
class Runner:
    ...
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, device='cuda', gpu_num=2):
        ...
        print("train_inner_iter, total_inner_iter",self.train_inner_iter,self.total_inner_iter)
        ...
        self.deviation_network = network_dp(
            SingleVarianceNetwork(**self.conf['model.variance_network']).to(self.device), gpu_num)
        self.color_network = network_dp(RenderingNetwork(**self.conf['model.rendering_network']).to(self.device),
                                        gpu_num)
        ...
        if is_continue:
            ...
            latest_model_name = "ckpt_SOMVideo_pre.pth"
        ...
        if data_type == 'multi_object_obman':
            #self.dataset = MultiObjectObmanDataset_Single(self.conf['dataset'], 'cpu', train_inner_iter=self.train_inner_iter, ray_batch_size=self.batch_size, start_index=7001, end_index=14000)
            self.dataset = MultiObjectObmanDataset_Single(self.conf['dataset'], 'cpu', train_inner_iter=self.train_inner_iter, ray_batch_size=self.batch_size, start_index=0, end_index=7000)
            # iter_step update in load_checkpoint
            train_sampler = get_iter_sampler(self.dataset, batch_size=self.scene_batch_size,drop_last=True, max_iteration=self.end_iter,start_iteration=self.iter_step)
            print('Initial data sampler from iter: %d.' % self.iter_step)
            self.dataloader = DataLoader(self.dataset,batch_sampler=train_sampler,num_workers=1)
            self.data_iter = enumerate(self.dataloader)
            print('Loader initialization: finished')
        elif data_type == 'ho3d':
            self.dataset = HO3DDataset(self.conf['dataset'], 'cpu', train_inner_iter=self.train_inner_iter,
                                       ray_batch_size=self.batch_size)
            # iter_step update in load_checkpoint
            ...
            self.dataset = DexYCBDataset(self.conf['dataset'], 'cpu', train_inner_iter=self.train_inner_iter,
                                         ray_batch_size=self.batch_size)
            # iter_step update in load_checkpoint
            train_sampler = get_iter_sampler(self.dataset, batch_size=self.scene_batch_size,
                                             drop_last=True, max_iteration=self.end_iter,
    ...
    def train(self):
        ...
        for iter_o in trange(0, res_step):
            ...
            self.writer.add_scalar('Loss/normal_smooth_loss', normal_smooth_loss * self.normal_smooth_weight,
                                   self.iter_step)
    ...
    def load_checkpoint(self, checkpoint_name):
        checkpoint = torch.load(os.path.join(self.base_exp_dir, checkpoint_name),
    ...
    def validate_image(self, scene, view_idx=-1, resolution_level=-1, test=False):
        ...
        image, dino_img, pose, intr = image.to(self.device), dino_img.to(self.device), pose.to(self.device), intr.to(
            self.device)
        latent = self.enc(image)
        glb, local = latent
        local = torch.cat([local, dino_img], dim=1)
                ...
                if len(out_rgb_fine) > 0:
                    cv.imwrite(os.path.join(self.base_exp_dir,
                               ...
                               np.concatenate([img_fine[..., i],
                                               self.dataset.image_at(scene, view_idx,
    ...
    def validate_mesh(self, scene, world_space=False, resolution=64, threshold=0.0, test=False, ref=None):
        ...
        if self.data_type == 'ho3d' or self.data_type == 'dexycb':

            image, dino_img, pose, intr, hA, hTo, image_p = self.dataset.gen_source_img_pose_intr(scene, test=test, ref=ref)

        else:

            image, dino_img, pose, intr, hA, hTo = self.dataset.gen_source_img_pose_intr(scene, test=test)
        ...
        jsTo = self.get_jsTx(hA.cpu(), hTo.cpu())
        jsTo = jsTo.to(self.device)
        vertices, triangles = self.renderer.extract_geometry(bound_min, bound_max, ref_feature=latent, jsTo=jsTo,pose=pose, intr=intr, resolution=resolution, threshold=threshold)
        end_time = time.time()
        process_time = end_time - start_time
        print("Process time: {:.2f} s\n".format(process_time))
        if not test:
            os.makedirs(os.path.join(self.base_exp_dir, 'meshes'), exist_ok=True)
        else:
            os.makedirs(os.path.join(self.base_exp_dir, 'meshes_test2'), exist_ok=True)
        ...
        if not test:
            ...
            if self.data_type == 'ho3d':
                ...
                image_n = image_n[:4]
                mesh.export(os.path.join(self.base_exp_dir, 'meshes_test2', '{}.ply'.format(scene_n + '_' + image_n)))
                ...
                scene_name = self.dataset.scene_lis[scene]
                mesh.export(os.path.join(self.base_exp_dir, 'meshes_test2', '{}.ply'.format(scene_name)))
        logging.info('End')
    # for psnr evalation
    def test_image(self, resolution_level=-1):
        ...
        if self.data_type == 'ho3d':
            ref_file_path = 'cache/ho3d_view_test.txt'
        elif self.data_type == 'dexycb':
            ref_file_path = 'cache/dexycb_view_test.txt'
        else:
            ref_file_path = 'cache/obman_view_test_all.txt'
        ...
        for scene, ref_idx, select_views, img_path in tqdm(ref_list):
            print(scene,ref_idx,select_views,img_path)
            ref_idx = int(ref_idx)
            select_views = [int(sv) for sv in select_views.split(',')]
            #print(scene, test, ref_idx)
            #image, dino_img, pose, intr, hA, hTo, _ = self.dataset.gen_source_img_pose_intr(scene, test, ref_idx)
            image, dino_img, pose, intr, hA, hTo = self.dataset.gen_source_img_pose_intr(scene, test)
            image, dino_img, pose, intr = image.to(self.device), dino_img.to(self.device), pose.to(
                self.device), intr.to(self.device)
            latent = self.enc(image)
            glb, local = latent
            local = torch.cat([local, dino_img], dim=1)
            glb = torch.cat([glb, dino_img.mean(dim=-1).mean(dim=-1)], dim=1)
            ...
            for view_idx in tqdm(select_views, leave=False):
                ...
                #if view_idx == ref_idx:
                #    continue
                print('select_views',select_views)
                print('view_idx',view_idx)
                print('images_lis',self.dataset.images_lis)
                view_src_path = self.dataset.images_lis[view_idx]
                save_view_path = os.path.join(self.base_exp_dir, 'views_fine_test',
                                              '{}_{}_{}_{}_{}_{}.png'.format(scene, ref_idx,
                                                                             *view_src_path.split('/')[-4:]))
                print(save_view_path)
    ...
    def inference_image(self, resolution_level=-1):
        ...
        for scene, ref_idx, select_views, img_path in tqdm(ref_list):
            ...
            for view_idx in tqdm(select_views, leave=False):
                ...
                save_view_path = os.path.join(self.base_exp_dir, 'views_fine_test','{}_{}_{}_{}_{}_{}.png'.format(scene, ref_idx,*view_src_path.split('/')[-4:]))
                ...
                for rays_o_batch, rays_d_batch in zip(rays_o, rays_d):
                    ...
                    weight_sum = render_out['weight_sum']
                    color_fine = render_out['color_fine']
                    #for both-rgb and obj-seg and 3rd-channel
...
if __name__ == '__main__':
    ...
    parser.add_argument('--conf', type=str, default='./confs/moo_wmask_dp_hand_test.conf')
    parser.add_argument('--mode', type=str, default='test_image')
    parser.add_argument('--mcube_threshold', type=float, default=0.0)
    parser.add_argument('--is_continue', default=True, action="store_true")
    parser.add_argument('--case', type=str, default='finetuning', help='split of obman')
    ...
    if args.mode == 'train':
        ...
        runner.test_image()
    elif args.mode == 'test_mesh_single':
        runner.test_mesh_single()
