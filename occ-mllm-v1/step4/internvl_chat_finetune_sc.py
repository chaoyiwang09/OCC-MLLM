...
from internvl.train.dataset import (ConcatDataset, TCSLoader,
                                    ...
                                    preprocess_phi3, preprocess_chaoyiSC)
...
from sc_trainer import ScTrainer
...
class TrainerConfig:
    ...
    weight_decay: float
    warmup_ratio: float
    num_epochs_stage_one: int
    num_epochs_stage_two: int
    ...
    beta_2: float
    alpha: float
    train_stage_type: str
    online_writeout_stage_one_path: str
    model_type: str
    template_name: str
    ...
    seed: int = 42  # 随机种子
    dataloader_num_workers: int = 4  # 数据加载的工作进程数
...
def concat_pad_data_collator_chaoyi(features, max_item_length=None, pad_id=0):
...
class LazySupervisedDataset(Dataset):
    ...
    def get_preprocess_function(self):
        ...
        #elif self.template_name == 'chaoyisc':
        #    preprocess_function = preprocess_chaoyiSC
        else:
    ...
    def multi_modal_get_item(self, data_item):
        ...
        # Calculate max sequence length that's divisible by batch_size
        #seq_length = (ret['attention_mask'].size(0) // 2) * 2  # Ensure divisible by batch_size=2
        
        
        # Create position ids that match the expected length
        #position_ids = torch.arange(seq_length, dtype=torch.long, device=ret['attention_mask'].device)
        #position_ids = position_ids[:seq_length]  # Truncate to exact length
        ...
        #print("chaoyi0",[data_item['conversations'][0]['value'],data_item['conversations'][2]['value']])
        #print("chaoyi1",[data_item['conversations'][1]['value'],data_item['conversations'][3]['value']])
        #print("position_ids content:", position_ids)  # See actual values
        ...
        #    pixel_values=pixel_values,
        #    image_flags=torch.tensor([1] * num_patches, dtype=torch.long),
        #    question=[data_item['conversations'][0]['value'],data_item['conversations'][2]['value']],
        #    answer=[data_item['conversations'][1]['value'],data_item['conversations'][3]['value']]
        #)
        #print([data_item['conversations'][0]['value'],data_item['conversations'][2]['value']])
        #print([data_item['conversations'][1]['value'],data_item['conversations'][3]['value']])
        #print("position_ids content:", position_ids)  # See actual values
...
def main():
    ...
    collator = concat_pad_data_collator_chaoyi
    #print(config.llm_config.model_type)

    SCoConfig = TrainerConfig(
        # 原有参数保持不变
        model_path=model_args.model_name_or_path,
        output_dir="/root/autodl-tmp/workspace/Base1_Stage1/InternVL/internvl_chat/shell/internvl2.0/2nd_finetune/outputdirSC",
        ...
        beta_2=0.999,
        alpha=0.1,
        train_stage_type="stage_one",
        online_writeout_stage_one_path="/root/autodl-tmp/workspace/Base1_Stage1/InternVL/internvl_chat/shell/internvl2.0/2nd_finetune/outputSCstage1",
        model_type=config.llm_config.model_type,
        template_name="chaoyisc",
